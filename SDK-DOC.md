...
- [TdObjectFactory](#tdobjectfactory)
  ...
  - [chatAdministratorRightsV2](#chatadministratorrightsv2)
  - [chatPermissions](#chatpermissions)
  - [chatPermissionsV2](#chatpermissionsv2)
...
- [TdSearch](#tdsearch)
  - [搜索本地聊天对象](#搜索本地聊天对象)
  - [搜索本地聊天对象V2](#搜索本地聊天对象v2)
  - [搜索聊天对象](#搜索聊天对象)
  - [搜索聊天对象V2](#搜索聊天对象v2)
  - [搜索用户名](#搜索用户名)
  - [搜索用户名或标题](#搜索用户名或标题)
  - [搜索用户名或标题V2](#搜索用户名或标题v2)
  - [搜索附近聊天](#搜索附近聊天)
  - [搜索手机号](#搜索手机号)
  - [搜索用户链接](#搜索用户链接)
- [TdFile](#tdfile)
...
| clientId     | 整数型   | 否   | 否   | 客户端ID                                                                         |
| chatId       | 长整数型 | 否   | 否   | 要转发消息的聊天标识符                                                           |
| messageId    | 长整数型 | 否   | 否   | 要转发的消息标识符列表。                                                         |
| revoke       | 逻辑型   | 否   | 是   | 传递true以删除所有聊天成员客户端中的消息。对于超级群组、频道和秘密聊天始终为true |
| isSync       | 逻辑型   | 否   | 是   | 是否同步执行，通过response返回结果。                                             |
| responseData | 整数型   | 是   | 是   | 接口返回的结果指针，该指针需要通过Free方法释放                                   |
| responseLen  | 整数型   | 是   | 是   | 接口返回的结果长度                                                               |
...
| messageThreadId | 长整数型 | 否   | 是   | 废弃；如果不为0，则为消息将发送的消息讨论标识符；仅适用于论坛讨论                                                                                    |
...
| chatId              | 长整数型 | 否   | 否   | 要转发消息的聊天标识符                                               |
| messageId           | 长整数型 | 否   | 否   | 要转发的消息标识符列表。                                             |
| disableNotification | 逻辑型   | 否   | 是   | 传入 true 可禁用关于置顶消息的通知。在频道和私聊中，通知始终会被禁用 |
| onlyForSelf         | 逻辑型   | 否   | 是   | 传入 true 表示仅为自己置顶该消息；仅适用于私聊                       |
| isSync              | 逻辑型   | 否   | 是   | 是否同步执行，通过response返回结果。                                 |
| responseData        | 整数型   | 是   | 是   | 接口返回的结果指针，该指针需要通过Free方法释放                       |
| responseLen         | 整数型   | 是   | 是   | 接口返回的结果长度                                                   |
...
> 说明：为聊天创建一个新的邀请链接。适用于基础群组、超级群组和频道。需要管理员权限以及聊天中的 can_invite_users 权限   
> 原名：TdGroup.createChatInviteLink  
> 返回：逻辑型  
...
| clientId           | 整数型   | 否   | 否   | 客户端ID                                                                                    |
| chatId             | 长整数型 | 否   | 否   | 聊天标识符                                                                                  |
| name               | 文本型   | 否   | 是   | 宽文本或utf8 邀请链接名称；0-32 个字符                                                      |
| expirationDate     | 整数型   | 否   | 是   | 邀请链接过期的时间点（Unix 时间戳）；如果永不过期则传递 0                                   |
| memberLimit        | 整数型   | 否   | 是   | 通过该链接同时加入聊天的最大成员数量；0-99999；如果不限制则传递 0                           |
| createsJoinRequest | 逻辑型   | 否   | 是   | 如果通过该链接加入聊天的用户需要管理员批准，则传递 true。在这种情况下 member_limit 必须为 0 |
| isSync             | 逻辑型   | 否   | 是   | 是否同步执行，通过response返回结果。                                                        |
| responseData       | 整数型   | 是   | 是   | 接口返回的结果指针，该指针需要通过Free方法释放                                              |
| responseLen        | 整数型   | 是   | 是   | 接口返回的结果长度                                                                          |
...
### chatAdministratorRightsV2

> 说明：；创建聊天管理员权限对象，描述管理员的权限，成功返回对象Id，失败返回0；若对象不再使用，需要调用FreeObject销毁   
> 原名：TdObjectFactory.chatAdministratorRightsV2  
> 返回：长整数型  
...
| canEditStories          | 逻辑型 | 否   | 是   | 如果管理员可以编辑其他用户发布的故事，将故事发布到聊天页面，固定聊天故事并访问故事存档，则为真；仅适用于超级群组和频道                                           |
| canDeleteStories        | 逻辑型 | 否   | 是   | 如果管理员可以删除其他用户发布的故事，则为真；仅适用于超级群组和频道                                                                                             |
| canManageDirectMessages | 逻辑型 | 否   | 是   | 如果管理员可以回复频道私信，则为 True；仅适用于频道。                                                                                                            |
| canManageTags           | 逻辑型 | 否   | 是   | 如果管理员可以更改其他用户的标签，则为 True；仅适用于基本群组和超级群组。                                                                                        |
| isAnonymous             | 逻辑型 | 否   | 是   | 如果管理员不在聊天成员列表中显示并匿名发送消息，则为真；仅适用于超级群组                                                                                         |
...
> 说明：；创建聊天用户权限对象，描述用户在聊天中允许执行的操作，成功返回对象Id，失败返回0；若对象不再使用，需要调用FreeObject销毁   
> 原名：TdObjectFactory.chatPermissionsV2  
> 返回：长整数型  
...
| canSendOtherMessages | 逻辑型 | 否   | 是   | 如果用户可以发送动画、游戏、贴纸和骰子并使用内联机器人，则为真               |
| canAddLinkPreviews   | 逻辑型 | 否   | 是   | 如果用户可以为其消息添加链接预览，则为真                                     |
| canReacToMessages    | 逻辑型 | 否   | 是   | 如果用户可以对消息做出反应，则为 True                                        |
| canEditTag           | 逻辑型 | 否   | 是   | 如果用户可以更改自身标签，则为 True。                                        |
| canChangeInfo        | 逻辑型 | 否   | 是   | 如果用户可以更改聊天标题、照片和其他设置，则为真                             |
| canInviteUsers       | 逻辑型 | 否   | 是   | 如果用户可以邀请新用户加入聊天，则为真                                       |
| canPinMessages       | 逻辑型 | 否   | 是   | 如果用户可以固定消息，则为真                                                 |
| canCreateTopics      | 逻辑型 | 否   | 是   | 如果用户可以创建主题，则为真                                                 |
...
| messageThreadId     | 长整数型 | 否   | 是   | 废弃；聊天讨论标识符；如果没有，则传递 null                                              |
...
> 说明：发送一条正在生成中的富消息草稿；仅适用于机器人；   
> 原名：TdMessageSent.sendRichMessageDraft  
> 返回：逻辑型  
...
| clientId     | 整数型   | 否   | 否   | 客户端ID                                               |
| chatId       | 长整数型 | 否   | 否   | 聊天标识符                                             |
| draftId      | 长整数型 | 否   | 否   | 该草稿的唯一标识符                                     |
| text         | 文本型   | 否   | 否   | HTML格式文本；宽文本或UTF8文本，发送内容；最大4096字符 |
| parseMode    | 整数型   | 否   | 是   | 0=无需解析                                             |
| forumTopicId | 整数型   | 否   | 是   | 消息将被发送的论坛话题标识符；如果没有则传0            |
| isSync       | 逻辑型   | 否   | 是   | 是否同步执行，通过response返回结果。                   |
| responseData | 整数型   | 是   | 是   | 接口返回的结果指针，该指针需要通过Free方法释放         |
| responseLen  | 整数型   | 是   | 是   | 接口返回的结果长度                                     |
...
> 说明：发送一条正在生成中的文本消息草稿；仅适用于机器人；   
> 原名：TdMessageSent.sendTextMessageDraft  
> 返回：逻辑型  
...
> 说明：调用流式草稿消息接口后，调用此接口确定最终消息   
> 原名：TdMessageSent.sendRichMessage  
> 返回：逻辑型  
...
| text         | 文本型   | 否   | 否   | HTML格式文本；宽文本或UTF8文本，发送内容；最大4096字符 |
| parseMode    | 整数型   | 否   | 是   | 0=无需解析                                             |
| clearDraft   | 逻辑型   | 否   | 是   | 传true以删除该聊天中的消息草稿                         |
| isSync       | 逻辑型   | 否   | 是   | 是否同步执行，通过response返回结果。                   |
| responseData | 整数型   | 是   | 是   | 接口返回的结果指针，该指针需要通过Free方法释放         |
| responseLen  | 整数型   | 是   | 是   | 接口返回的结果长度                                     |
...
| messageThreadId | 长整数型 | 否   | 是   | 废弃；当type为1=返回可在超组中提及的用户时可用，当前聊天讨论Id                                                                                                                                                                                                                                                   |
...
| chatId       | 长整数型 | 否   | 否   | 聊天标识符                                                                             |
| userId       | 长整数型 | 否   | 否   | 成员标识符                                                                             |
| customTitle  | 文本型   | 否   | 是   | 废弃；宽文本或UTF8文本；管理员的自定义标题；0-16个字符，不含表情符号；仅适用于超级群组 |
...
> 说明：通过用户名或名称搜索已知聊天对象（处在聊天列表的）   
> 原名：TdSearch.searchChatsOnServerV2  
> 返回：逻辑型  
...
> 说明：通过用户名或标题搜索公开用户、超级群和频道；返回匹配的结果；   
> 原名：TdSearch.searchPublicChatsV2  
> 返回：逻辑型  
